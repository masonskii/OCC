...
def execute_code(params: dict = None) -> str:
    if not params:
        return ""
...
def execute_c_code(params: dict) -> [str, str or None]:
    ...
    process: subprocess.CompletedProcess[str] = subprocess.run(execute_cmd, capture_output=True, text=True)
    logging.info(f'Code executed, stdout: {process.stdout.strip()}, stderr: {process.stderr.strip() or None}')
...
def execute_cs_code(params: dict) -> [str, str or None]:
    ...
    subprocess.run(['dotnet', 'new', 'console', '-o', path, '--force'], check=True)
    # Write the code to a file
